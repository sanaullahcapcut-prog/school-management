...
export default function QuickAddSimple() {
  ...
  async function onSave() {
    ...
    if (!Number.isFinite(n) || n <= 0) { setMsg("Enter a valid amount (> 0)"); return; }
    setMsg("Saving…");
    try {
      await insertTransaction(type, n, note);
      setAmount(""); setNote("");
      setMsg("Saved ✓"); setTimeout(()=>setMsg(""), 1000);
    } catch (e:any) {
  ...
  return (
    <div className="p-4 border rounded-xl max-w-xl">
      ...
      <div className="flex gap-2 mb-3">
        <select className="border rounded px-2 py-1" value={type}
          onChange={(e)=>setType(e.target.value as "credit"|"debit")}>
          <option value="credit">Credit</option>
          <option value="debit">Debit</option>
        </select>
        <input className="border rounded px-2 py-1 w-36" type="text" inputMode="decimal"
          placeholder="Amount" value={amount} onChange={(e)=>setAmount(e.target.value)} />
        <input className="border rounded px-2 py-1 flex-1" placeholder="Note"
